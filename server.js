 ...
 *
 * HOSTED AT: https://finance.carltonedu.com
 */
...
// ── MONGODB ───────────────────────────────────────────────────
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/delta-finance';
...
const SettingsModel    = mongoose.model('Settings',    new mongoose.Schema({ _id: String }, raw));
const UserModel        = mongoose.model('User',        new mongoose.Schema({ id: String }, raw));
const AccountModel     = mongoose.model('Account',     new mongoose.Schema({ id: String }, raw));
const StudentModel     = mongoose.model('Student',     new mongoose.Schema({ id: String }, raw));
const ProgramModel     = mongoose.model('Program',     new mongoose.Schema({ id: String }, raw));
const PaymentModel     = mongoose.model('Payment',     new mongoose.Schema({ id: String }, raw));
const ExpenseModel     = mongoose.model('Expense',     new mongoose.Schema({ id: String }, raw));
const StaffModel       = mongoose.model('Staff',       new mongoose.Schema({ id: String }, raw));
const TransferModel    = mongoose.model('Transfer',    new mongoose.Schema({ id: String }, raw));
const PayrollModel     = mongoose.model('PayrollRun',  new mongoose.Schema({ id: String }, raw));
const CategoryModel    = mongoose.model('Category',    new mongoose.Schema({ id: String }, raw));
...
async function readDB() {
  const [
    settingsDoc, users, accounts, students, programs,
    payments, expenses, staff, transfers, payrollRuns, categories
  ] = await Promise.all([
  ...
  const settings = settingsDoc
    ...
    : { academyName: 'Delta Academy', rates: { usdAed: 3.67, usdInr: 83.5 } };
...
async function writeDB(data) {
  ...
  if (data.settings) {
    ops.push(
      SettingsModel.findByIdAndUpdate('main', { ...data.settings, _id: 'main' }, { upsert: true, new: true })
    );
  }
  ...
  for (const [Model, arr] of pairs) {
    ...
    ops.push(
      Model.deleteMany({}).then(() =>
        arr.length > 0 ? Model.insertMany(arr, { ordered: false }) : Promise.resolve()
      )
    );
  }
...
function loadSmtpConfig() {
  try {
    if (fs.existsSync(SMTP_FILE))
      return { ...DEFAULT_SMTP, ...JSON.parse(fs.readFileSync(SMTP_FILE, 'utf8')) };
...
app.post('/smtp-config', async (req, res) => {
  ...
  if (!host || !port || !user || !pass)
    return res.status(400).json({ error: 'host, port, user and pass are required' });
...
<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto">
  <div style="background:#0f0f0f;padding:20px 28px;border-radius:10px 10px 0 0">
    <span style="font-size:20px;font-weight:700;color:#fff">
      ...
      <span style="color:#fff;font-size:14px;letter-spacing:2px"> ACADEMY</span>
    </span>
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
  <div style="background:#0f0f0f;padding:18px 28px;border-radius:10px 10px 0 0">
    <span style="font-size:18px;font-weight:700;color:#fff">
      <span style="color:#d42a2a">d</span><span style="color:#7b2cbf">e</span><span style="color:#1e3fcf">l</span><span style="color:#1e7d3b">t</span><span style="color:#f5a623">a</span>
      <span style="color:#fff;font-size:13px;letter-spacing:2px"> ACADEMY</span>
    </span>
  ...
  <div style="text-align:center;padding:12px;font-size:11px;color:#aaa">Delta Trading Academy · no-reply@deltaacademy.ae</div>
</div>`;

  try {
    await transporter.sendMail({
      from: '"Delta Academy" <no-reply@deltaacademy.ae>',
      to, subject, text: text || '', html: emailHtml
    });
...
app.listen(PORT, () => {
  console.log(`\n${'━'.repeat(46)}`);
  console.log(`  Delta Academy Finance Server`);
  console.log(`  Port    : ${PORT}`);
  console.log(`  Local   : http://localhost:${PORT}`);
  console.log(`  Live    : https://finance.carltonedu.com`);
